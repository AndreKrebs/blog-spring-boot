...
import br.eti.krebscode.blogjhipster.web.rest.errors.BadRequestAlertException;
import br.eti.krebscode.blogjhipster.web.rest.util.HeaderUtil;
import br.eti.krebscode.blogjhipster.web.rest.util.PaginationUtil;
...
public class EntryResource {
    ...
    public EntryResource(EntryService entryService, EntryQueryService entryQueryService) {
     ...
     */
    @GetMapping("/entries")
    public ResponseEntity<List<EntryDTO>> getAllEntries(EntryCriteria criteria, Pageable pageable) {
        log.debug("REST request to get Entries by criteria: {}", criteria);
        Page<EntryDTO> page = entryQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/entries");
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
    * GET  /entries/count : count all the entries.
    *
    * @param criteria the criterias which the requested entities should match
    * @return the ResponseEntity with status 200 (OK) and the count in body
    */
    @GetMapping("/entries/count")
    public ResponseEntity<Long> countEntries(EntryCriteria criteria) {
        log.debug("REST request to count Entries by criteria: {}", criteria);
        return ResponseEntity.ok().body(entryQueryService.countByCriteria(criteria));
    }
